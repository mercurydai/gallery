 ...
 * @version $Id: cache.php,v 1.5 2010/02/14 12:14:40 andig2 Exp $
 */
...
{
#   if (!ereg('/$', $cache_folder)) $cache_folder .= '/';
    $stamp       = $cache_folder.'cache_last_purge';
    $cache_mtime = @filemtime($stamp); // get time the cache was last purged (once a day)
#dlog(sprintf('cache_prune_folder %s, %b, %b', $cache_folder, $force_prune, $simulate));

    // if cache was last purged a day or more ago
    if ($force_prune || time() - $cache_mtime > $cache_max_age/2) # 86400)
    {
