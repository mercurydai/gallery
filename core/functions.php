 ...
 * @version $Id: functions.php,v 1.119 2010/11/22 10:44:33 andig2 Exp $
 */
...
// PHP 5.3 compatibility fix: set_magic_quotes_runtime(0);
...
$smarty->allow_php_tag  = 1;                            // changed as of Smarty 3
...
if ($config['moz_am']) {
    header('X-Account-Management: http://localhost'.dirname($_SERVER['PHP_SELF']).'/amcd.json');
}
...
function auth_check($redirect = true)
{
    ...
    if ($config['multiuser'] && ($_COOKIE['VDBuserid'] !== $config['guestid']))
    {
        ...
        if (preg_match('/[a-z]+/i', $user) && preg_match('/[0-9]+/', $pass) && is_numeric($userid))
        {
            ...
            $res    = runSQL("SELECT cookiecode FROM ".TBL_USERS." WHERE name='".mysql_escape_string($user)."' AND id=$userid");
            $result = $res[0]['cookiecode'] == $pass;
        }

        // HTTP basic authentication (for RSS feed)?

        // Hack für mod_fastcgi [muddle @ 2010-01-17]:
        ...
        if (!$result && isset($_SERVER['PHP_AUTH_USER']) && isset($_SERVER['PHP_AUTH_PW']))
        {
            ...
            if (preg_match('/[a-z]/i', $user))
            {
                ...
                $res    = runSQL("SELECT * FROM ".TBL_USERS." WHERE name='".mysql_escape_string($user)."'");
	...
	// !! experimental Mozilla Account Management code
    if ($config['moz_am']) {
        if ($result) {
            header('X-Account-Management-Status: active; name="'.$_COOKIE['VDBusername'].'"');
        } else {
